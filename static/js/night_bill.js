...
async function generateReport() {
    ...
    try {
        const res = await fetch('/api/night_bill', {
            method: 'POST',
        ...
        if (data.rows && data.rows.length > 0) {
            ...
            tableBody.innerHTML = '<tr><td colspan="9" class="empty-state">No employees eligible for Night Bill found (Out-time >= 10:30 PM).</td></tr>';
        }
    } catch (err) {
